...
describe('Vault', () => {
  ...
  const _getCollateral = async (account, collateral) => {
    ...
    if (collateral.sub(remainingCTK) > new BN(0))
      await this.ctk.transfer(account, collateral.sub(remainingCTK), { from: other });
  };
  ...
  describe('Check initializations', () => {
    ...
    it('should check collateral per unit', async () => {
      expect((await this.vault.collateralPerUnit()).toNumber()).to.equal(10000000);
    });
  ...
  describe('Mint position tokens', () => {
    ...
    it('should charge 0 fee from automated market maker', async () => {
      ...
      const receipt = await this.vault.mintPositions(tokenAmount, { from: amm });
  ...
  describe('Update spot', () => {
    ...
    it('should settle contract on breach', async () => {
      const receipt = await this.vault.updateSpot(breachedSpot, { from: oracle });

      expectEvent(receipt, 'ContractSettled', { _settlePrice: new BN(breachedSpot) });

      expect(await this.vault.isSettled()).to.equal(true);
      expect((await this.vault.contractName()).toString()).to.equal(`${vaultName} (settled)`);
      expect((await this.vault.settlementPrice()).toNumber()).to.equal(breachedSpot);
    });
  ...
  describe('Claim fee', () => {
    ...
    it('should allow owner to claim accumulated fee', async () => {
      ...
      expectEvent(receipt, 'FeeClaimed', { _payee: payee, _weiAmount: new BN(expectedFee) });

      expect((await this.ctk.balanceOf(this.vault.address)).toNumber()).to.equal(
        initialCTK - expectedFee
      );
      expect((await this.ctk.balanceOf(payee)).toNumber()).to.equal(expectedFee);
    });
  });
  ...
  describe('Settle positions', () => {
    ...
    it('should reject if contract not settled', async () => {
      await expectRevert(this.vault.settlePositions({ from: user }), 'Contract should be settled');
    });
  ...
  describe('Update automated market maker', () => {
    ...
    it('should be able to update oracle', async () => {
      const receipt = await this.vault.updateAutomatedMarketMaker(newAMM, { from: owner });

      expectEvent(receipt, 'AutomatedMarketMakerUpdated', {
        _previousAMM: amm,
        _newAMM: newAMM,
      });
      expect((await this.vault.automatedMarketMaker()).toString()).to.equal(newAMM);
    });
  });
});
