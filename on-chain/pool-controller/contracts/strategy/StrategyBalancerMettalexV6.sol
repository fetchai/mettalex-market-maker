...
contract StrategyBalancerMettalexV6 {
    ...
    //identifier for tracking commodities  
    uint256 public commodity_id;
    address public want;
    address public balancer;
    // struct variable of commodity pool having info of vault, virtual long and short tokens
    struct CPoolInfo {
        uint256 balance;
        address mettalexVault;
        address longToken;
        address shortToken;
        uint256 vLtk;
        uint256 vStk;
        bool isBreachHandled;
    }

    CPoolInfo[] public cpoolInfo;
    ...
    //  */
    // modifier notSettled {
    //     for(uint256 i=0;i<cpoolInfo.length();i++){
    //     IMettalexVault mVault = IMettalexVault(cpoolInfo[i].mettalexVault);
    ...
    modifier isBPoolBind(uint256 _cid){
        _bpoolbind(_cid);
        _;
        _unbind(_cid);
    }
    ...
    function deposit() external {
        ...
        uint256[] memory prevBalance; 

        for(uint256 i=0;i<cpoolInfo.length;i++){

            // IMettalexVault mVault = IMettalexVault(cpoolInfo[i].mettalexVault);

            if(IMettalexVault(cpoolInfo[i].mettalexVault).isSettled()){
    
                prevBalance[i] = cpoolInfo[i].balance; 

                //Logic for dividing liquidity into multiple logically-created commodity pool
                cpoolInfo[i].balance = cpoolInfo[i].balance.add(IERC20(want).balanceOf(address(this))
                .mul(TargetAllocation[i]).div(100));
            
                _depositInternal(cpoolInfo[i].balance-prevBalance[i], cpoolInfo[i]);
            }
            else{
                cpoolInfo[i].balance = cpoolInfo[i].balance.add(IERC20(want).balanceOf(address(this))
                .mul(TargetAllocation[i]).div(100));
    ...
    function withdraw(uint256 _amount) external {
        ...
        for(uint256 i=0;i<cpoolInfo.length;i++){
            ...
            if (mVault.isSettled()) {
                ...
                // _amount
                // );
                cpoolInfo[i].balance-=_amount.mul(TargetAllocation[i]).div(100);
            } else {
            // _unbind(_id);
                _redeemPositions(i);
                   }
            ...
            _depositInternal(prevBalance[i]-cpoolInfo[i].balance, cpoolInfo[i]);
    ...
    function withdraw(address _token) external returns (uint256 balance) {
        ...
        //Checking long and short tokens for all the commodities 
        for(uint256 i=0;i<cpoolInfo.length;i++){
        require(address(_token) != cpoolInfo[i].longToken, "LTOK");
        require(address(_token) != cpoolInfo[i].shortToken, "STOK");
        }
     ...
     * @return tokenAmountOut of _to token returned to user
     * @return spotPriceAfter The spot price after token swap
     */
    ...
    function _swapFromCoin(
        ...
        require(
            tokenOut == cpoolInfo[cid].longToken || tokenOut == cpoolInfo[cid].shortToken,
            "ERR_TOKEN_OUT"
        );
        ...
        //Rebalance Pool
        updateSpotAndNormalizeWeights(cid);
    ...
    function _swapToCoin(
        ...
        require(tokenIn == cpoolInfo[cid].longToken || tokenIn == cpoolInfo[cid].shortToken, "ERR_TOKEN_IN");
     ...
     */
    function handleBreach(uint256 _cid) public {

        CPoolInfo memory _cpool = cpoolInfo[_cid];
        IMettalexVault mVault = IMettalexVault(_cpool.mettalexVault);
        require(mVault.isSettled(), "mVault should be settled");
        require(!breaker, "!breaker");
        if (!_cpool.isBreachHandled) {
            _cpool.isBreachHandled = true;
     ...
     * @dev _id index of commodity pool
    ...
    function updateCommodityAfterBreach(
        ...
        bool hasLong = IERC20(_cpool.longToken).balanceOf(address(this)) > 0;
        bool hasShort = IERC20(_cpool.shortToken).balanceOf(address(this)) > 0;
        if (hasLong || hasShort) {
            handleBreach(_cid);
        }

        _cpool.mettalexVault = _vault;
        _cpool.longToken = _ltk;
        _cpool.shortToken = _stk;
        
        _depositInternal(_cpool.balance, _cpool);

        _cpool.isBreachHandled = false;
    }
    ...
    function balanceOf() external view returns (uint256 total) {
        ...
        for(uint256 i=0;i<cpoolInfo.length;i++){
        //Balance of strategy
        (uint256 stkPrice, uint256 ltkPrice) = _getSpotPrice(i);

        uint256 stkBalance = IERC20(cpoolInfo[i].shortToken).balanceOf(address(this)).mul(
            stkPrice
        );
        uint256 ltkBalance = IERC20(cpoolInfo[i].longToken).balanceOf(address(this)).mul(
            ltkPrice
        );

        total = total.add((stkBalance).add(ltkBalance));
        }

        total = total.add(IERC20(want).balanceOf(address(this)));
    ...
    /**
     * @dev Used to deploy new commodity 
     * @dev Can be called by governance only
     * @param _mettalexVault, _longToken, _shortoken,  addresses of vault, LTK/STK of new commodity
    //  */
    function setNewMarket(address _mettalexVault, address _longToken, address _shortToken)public {
        require(msg.sender == governance, "!governance");
        cpoolInfo.push(CPoolInfo({
            mettalexVault: _mettalexVault,
            longToken: _longToken,
    ...
    function getExpectedOutAmount(
        ...
        // uint256 tokenBalanceIn = bpool.getBalance(fromToken) //holdings of long tokens in strategy for that commodity;
        // uint256 tokenBalanceOut = bpool.getBalance(toToken); 
        
         // Get AMM Pool token balances
        uint256[3] memory bal;
        bal[0] = IERC20(_cpool.longToken).balanceOf(address(this));
        bal[1] = IERC20(_cpool.shortToken).balanceOf(address(this));
        bal[2] = IERC20(want).balanceOf(address(this));

        uint256 spotPrice = IMettalexVault(_cpool.mettalexVault).priceSpot();
        uint256[3] memory wt = IStrategyHelper(strategyHelper).CalcDenormWeights(bal, spotPrice, _cpool.mettalexVault);
        ...
        if(fromToken == want){
            tokenBalanceIn = bal[2];
            tokenWeightOut = wt[2];
            if(toToken == _cpool.longToken){
                tokenBalanceOut = _cpool.vLtk.add(bal[0]);
                tokenWeightOut = wt[0];
            }
            else if(toToken == _cpool.shortToken){
                tokenBalanceOut = _cpool.vStk.add(bal[1]);
                tokenWeightOut = wt[1];
            }
        } else if(fromToken == _cpool.longToken){
                tokenBalanceIn = _cpool.vLtk.add(bal[0]);
                tokenWeightIn = wt[0];
                if(toToken == want){
                    tokenBalanceOut = bal[2];
                    tokenWeightOut = wt[2];
                }
                else if(toToken == _cpool.shortToken){
                    tokenBalanceOut = _cpool.vStk.add(bal[1]);
                ...
                tokenBalanceIn = _cpool.vStk.add(bal[1]);
                tokenWeightIn = wt[1];
                if(toToken == _cpool.longToken){
                    tokenBalanceOut = _cpool.vLtk.add(bal[0]);
                    tokenWeightOut = wt[0];
                }
                else if(toToken == want){
                    tokenBalanceOut = bal[2];
                    tokenWeightOut = wt[2];
                }
        }
    ...
    function _calculateSpotPrice(uint _cid) internal view returns (uint256 spotPrice) {
        ...
        //spot price calculation using Vault
        uint256 collateralPerUnit = IMettalexVault(_cpool.mettalexVault)
                .collateralPerUnit();
           
        uint256 spot = IMettalexVault(_cpool.mettalexVault).priceSpot();

        uint256 priceShort = collateralPerUnit.mul(
                (spot.sub(floor)).div(cap.sub(floor))
            );
        uint256  priceLong = collateralPerUnit.mul(
     ...
     */
    function _getSpotPrice(uint256 _cid)
        internal
        view
        returns (uint256 shortPrice, uint256 longPrice)
    {
        ...
        if (isBound(want)) {
            shortPrice = IBalancer(balancer).getSpotPrice(want, _cpool.shortToken);
            longPrice = IBalancer(balancer).getSpotPrice(want, _cpool.longToken);
        } else {
            uint256 collateralPerUnit = IMettalexVault(_cpool.mettalexVault)
                .collateralPerUnit();
            uint256 cap = IMettalexVault(_cpool.mettalexVault).priceCap();
            uint256 floor = IMettalexVault(_cpool.mettalexVault).priceFloor();
            uint256 spot = IMettalexVault(_cpool.mettalexVault).priceSpot();
    ...
    function _redeemPositions(uint256 _id) internal {

        CPoolInfo memory _cpool = cpoolInfo[_id];
        IMettalexVault mVault = IMettalexVault(_cpool.mettalexVault);
        uint256 ltkQty = IERC20(_cpool.longToken).balanceOf(address(this));
        uint256 stkQty = IERC20(_cpool.shortToken).balanceOf(address(this));
        if (stkQty < ltkQty) {
            if (stkQty > 0) {
     ...
     */
    function updateSpotAndNormalizeWeights(uint256 cid) public {
        CPoolInfo memory _cpool = cpoolInfo[cid];
        IMettalexVault mVault = IMettalexVault(_cpool.mettalexVault);
        require(!mVault.isSettled(), "mVault should not be settled");

        _rebalance(IMettalexVault(cpoolInfo[cid].mettalexVault).priceSpot(), cid);
    }

    function _rebalance(uint256 spotPrice, uint256 cid) internal {

        CPoolInfo memory _cpool = cpoolInfo[cid];
        // Get AMM Pool token balances
        uint256[3] memory bal;
        bal[0] = IERC20(_cpool.longToken).balanceOf(balancer);
        bal[1] = IERC20(_cpool.shortToken).balanceOf(balancer);
        bal[2] = IERC20(want).balanceOf(balancer);

        // Re-calculate de-normalised weights
        uint256[3] memory newWt = IStrategyHelper(strategyHelper).CalcDenormWeights(bal, spotPrice, _cpool.mettalexVault);

        address[3] memory tokens = [_cpool.longToken, _cpool.shortToken, want];
    ...
    function _depositInternal(uint256 _amount, CPoolInfo memory _cpoolInfo) private returns (bool){

        // Get deposit amount and allocate half to minting position tokens
            
            uint256 wantBeforeMintandDeposit = _amount;
            
            uint256 wantToVault = wantBeforeMintandDeposit.div(2); 
            
            uint256 positionsExpected = wantToVault.div(
            IMettalexVault(_cpoolInfo.mettalexVault).collateralPerUnit()
        );

            _cpoolInfo.vLtk = _cpoolInfo.vLtk.add(positionsExpected); 
            _cpoolInfo.vStk = _cpoolInfo.vStk.add(positionsExpected);
            _cpoolInfo.balance = _cpoolInfo.balance.sub(wantToVault);

             // // Get AMM Pool Position token balances
            uint256 balancerLtk = 0;
            uint256 balancerStk = 0;

        // // Get strategy balance
            uint256 strategyLtk = IERC20(_cpoolInfo.longToken).balanceOf(address(this));
            uint256 strategyStk = IERC20(_cpoolInfo.ShortToken).balanceOf(address(this));
     ...
     function _bpoolbind(uint256 index) private{

        CPoolInfo memory _cpool = cpoolInfo[index];

        uint256 wantToVault = _cpool.vLtk.mul(IMettalexVault(_cpool.mettalexVault).collateralPerUnit()); 

        IERC20(want).safeApprove(_cpool.mettalexVault, 0);

        IERC20(want).safeApprove(_cpool.mettalexVault, wantToVault);

        IMettalexVault(_cpool.mettalexVault).mintFromCollateralAmount(wantToVault);
        
        //After minting, updating virtual Long and short tokens
       _cpool.vLtk = 0;
       _cpool.vStk = 0;
        ...
        // uint256 strategyWant = _cpool.balance;
         uint256 strategyWant = IERC20(want).balanceOf(address(this));
         uint256 strategyLtk = IERC20(_cpool.longToken).balanceOf(address(this));
         uint256 strategyStk = IERC20(_cpool.shortToken).balanceOf(address(this));

        // // Approve transfer to balancer pool
         IERC20(want).safeApprove(balancer, 0);
        //  IERC20(want).safeApprove(balancer, strategyWant);
         IERC20(want).safeApprove(balancer, _cpool.balance);
         IERC20(_cpool.longToken).safeApprove(balancer, 0);
         IERC20(_cpool.longToken).safeApprove(balancer, strategyLtk);
         IERC20(_cpool.shortToken).safeApprove(balancer, 0);
         IERC20(_cpool.shortToken).safeApprove(balancer, strategyStk);
        ...
        // // While calculating weights, consider all ( balancer + strategy ) tokens to even out the weights
         ...
         uint256[3] memory wt = IStrategyHelper(strategyHelper).CalcDenormWeights(
             bal,
             IMettalexVault(_cpool.mettalexVault).priceSpot(),
             _cpool.mettalexVault
         );

         IBalancer bPool = IBalancer(balancer);
         // Rebind tokens to balancer pool again with newly calculated weights
         bool isWantBound = bPool.isBound(want);
         bool isStkBound = bPool.isBound(_cpool.shortToken);
         bool isLtkBound = bPool.isBound(_cpool.longToken);

         if (!isStkBound && !isLtkBound && !isWantBound) {
             bPool.bind(_cpool.shortToken, bal[0], wt[0]);
             bPool.bind(_cpool.longToken, bal[1], wt[1]);
             bPool.bind(want, bal[2], wt[2]);
         } else {
             
             int256[3] memory delta;
             address[3] memory tokens = [_cpool.shortToken, _cpool.longToken, want];
       ...
       function _unbind(uint256 index) internal {
        // Unbind tokens from Balancer pool
        CPoolInfo memory _cpool = cpoolInfo[index];
        IBalancer bPool = IBalancer(balancer);
         //isBound condition
        ...
        if (isStkBound && isLtkBound && isWantBound){
            ...
            for (uint256 i = 0; i < tokens.length; i++) {
                //updating balance variable of _cpool
                // if(tokens[i] == want){
                //     _cpool.balance = _cpool.balance.add(IERC20(tokens[i]).balanceOf(balancer));
                // }
    ...
    function _settle(uint256 _id) internal {
        ...
        IMettalexVault mVault = IMettalexVault(_cpool.mettalexVault);
        require(mVault.isSettled(), "mVault should be settled");
        uint256 prevWant = IERC20(want).balanceOf(address(this));
        mVault.settlePositions();
        uint256 currentWant = IERC20(want).balanceOf(address(this));
        _cpool.balance += currentWant.sub(prevWant);
